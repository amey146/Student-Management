    ...
    if request.method == 'POST':
        ...
        for key, value in request.POST.items():
            if key.startswith('scoredmarks_'):
                ...
                # Collect quiz_score data to save in CSV
                quiz_data.append([today, sbname, student_id, score])
        # Save the quiz_score data to CSV
        ...
        return render(request, 'quiz_score/emailsent.html')

    students = Student.objects.all()
    return render(request, "quizmgt/quiz_home.html")
    return render(request, 'quiz_score.html', {'students': students})


# Function to save quiz_score to CSV
def save_scores_to_csv(quiz_data, sbname):
    new_file_start = get_week_start()
    # Specify the subdirectory 'quiz_score'
...
def quiz_list(request):
    # Update the path to include 'quiz_score' subdirectory
...
def download_csv(request, file_name):
    # Update to include 'quiz_score' subdirectory
    media_root = os.path.join(settings.MEDIA_ROOT, 'quiz_score')
